...
class CTABGANPlusSynthesizer:
    ...
    def sample(self, n, seed=0):
        ...
        print(f'Generated data len: {len(data)}')
        result,resample = self.transformer.inverse_transform(data)
        print(f'Transformed data len: {len(result)}')

        # It has a cap on 600 seconds. Thats why it fails after 10 minutes.
        t0 = time.time()
        while len(result) < n and (time.time() - t0) <= 600:
            ...
            print(f'Generated resampled data len: {len(data_resample)}')

            res,resample = self.transformer.inverse_transform(data_resample)
            print(f'Transformed resampled data len: {len(res)}')
